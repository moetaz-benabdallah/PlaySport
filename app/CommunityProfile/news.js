...
.controller('newsCtrl', function($scope, News,$http,$route,$routeParams) {
    $scope.newsData = {};
    $scope.newNews = function() {
      $scope.newsData.communityName=$routeParams.param ;
    var news = new News($scope.newsData);
    news.$save();
    $route.reload();  
  }  
    
      $http.get("http://localhost:4000/tweeter/news/community/"+$routeParams.param)
